...
for (i in 1:4) {
  ...
  svg(paste("radar_plot_",name[[1,1]],name[[2,1]],".svg",sep=""), 
      bg="transparent")
  radarchart(df, axistype=0 , plwd=4 , plty=1, pcol=c("mediumpurple2","tomato3"),
             cglcol="black", cglty=2, axislabcol="black", cglwd=2,vlcex=0.8 )
    legend(x=0.8, y=1.2, legend = c(name[[1,1]], name[[2,1]]), 
           col=c("mediumpurple2","tomato3"), lty=1, lwd=2, cex=0.8)
  dev.off()
  }
